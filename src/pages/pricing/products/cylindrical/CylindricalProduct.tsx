...
type CylindricalProductProps = {}

const PRODUCT_NAME = 'CYLINDRICAL'

const CylindricalProduct = (props: CylindricalProductProps) => {
	const { data, options, loading } = useData(PRODUCT_NAME)
	const [showForm, setShowForm] = React.useState<boolean>(false)
	const [messageApi, contextHolder] = message.useMessage()

	const postValidation = Yup.object().shape({
		cyndicalSize: Yup.string().required('This field is required.'),
			...
			{options?.length ? (
				<Formik
					...
					onSubmit={(values: any, { setSubmitting }) => {
						postData('/api/admin/add/cyndical', values)
							.then((success) => {
								setSubmitting(false)
								setShowForm(false)
								messageApi.open({
...
export default CylindricalProduct
