...
const PRODUCT_NAME = 'J-BOLT'

const JBoltProduct = (props: JBoltProductProps) => {
	const { data, options, loading } = useData(PRODUCT_NAME)
	const [showForm, setShowForm] = React.useState<boolean>(false)
	const [messageApi, contextHolder] = message.useMessage()

	const postValidation = Yup.object().shape({
		Lenght: Yup.string().required('This field is required.'),
			...
			{options?.length ? (
				<Formik
					...
					onSubmit={(values: any, { setSubmitting }) => {
						postData('/api/admin/add/Jbolt', values)
							.then((success) => {
								setSubmitting(false)
								setShowForm(false)
								messageApi.open({
